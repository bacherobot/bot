const { ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, EmbedBuilder, SlashCommandBuilder, time, AttachmentBuilder } = require('discord.js')
const bacheroFunctions = require('../../functions')
const escape = require('markdown-escape')
const { Util } = require('quickmongo')
module.exports = {
	slashInfo: new SlashCommandBuilder()
		...
		.addAttachmentOption(option =>
			option
				.setName("image")
				.setDescription("Image du cadeau a gagner")
				.setRequired(false))
		.addIntegerOption(option =>
			option
	...
	async execute(interaction) {
		...
		if (interaction.options.getInteger('min_participants')) embed.addFields({ name: "Nombre minimum de participants.", value: `${interaction.options.getInteger('min_participants')}`, inline: true })
		if (attachment) embed.setImage(attachment.url) // embed.setImage(attachment.url)
