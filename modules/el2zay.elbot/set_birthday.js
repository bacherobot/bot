...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!isValidDate(date)) {
            return await interaction.editReply({ content: "La date n'est pas valide.", ephemeral: true })
        }

        // Si l'année date de moins de 10 ans faire une blague
        if (annee > new Date().getFullYear() - 10) {
            interaction.editReply({ content: "Votre compte sera signalé à Discord et vous serez banni de Discord.", ephemeral: true })
            setTimeout(() => {
                interaction.editReply({ content: "Nan je déconne, je suis pas un bot de modération.", ephemeral: true })
            }
                , 5000)
        } else {
            interaction.editReply({ content: `Votre anniversaire est le ${jour}/${mois}/${annee}`, ephemeral: true })
        }
    }
    //  Ça la flemme de le finir mais juste rajouter les annivs a la bdd
}
