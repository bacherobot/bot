...
module.exports = {
    slashInfo: new SlashCommandBuilder()
        ...
        .addStringOption(option => option
            .setName('option1')
            .setDescription('Première choix d\'option')
            .setMaxLength(200)
            .setRequired(true)
        )
        .addStringOption(option => option
        ...
        .addStringOption(option => option.setName('type')
            .setDescription("Type de durée avant que le sondage ne se termine")
            .setChoices(
    ...
    async interactionListener(listener) {
        listener.on('button', async (interaction) => {
            ...
            if (interaction.customId.startsWith('sondageCmd-one-') || interaction.customId.startsWith('sondageCmd-two-')) {
                ...
                if (!sondage) return await interaction.editReply({ content: "Ce sondage n'existe plus !", ephemeral: true })

                // Choix de l'utilisateur
                var userChoice = interaction.customId.split('-')[1].replace('one', '1').replace('two', '2')

                // Si l'utilisateur a déjà voté
                if (sondage.votants.length && sondage.votants.find(v => v.id == interaction.user.id)) {
                    // S'il a voté pour la même option
                    if (sondage.votants.find(v => v.id == interaction.user.id).choice == userChoice) return await interaction.editReply({ content: "Vous avez déjà voté pour cette option !", ephemeral: true })

                    // S'il a voté pour une autre option
                    else {
                        // Ajouter le vote
                        sondage['choix_vote' + sondage.votants.find(v => v.id == interaction.user.id).choice]-- // on enlève le vote de l'ancienne option
                        sondage['choix_vote' + userChoice]++ // on ajoute le vote à la nouvelle option
                        sondage.votants.find(v => v.id == interaction.user.id).choice = userChoice // on change le choix de l'utilisateur
            ...
            if (interaction.customId.startsWith('sondageCmd-edit-')) {
                ...
                if (!interaction.channel.permissionsFor(interaction.member).has(PermissionFlagsBits.ManageMessages)) {
                    return interaction.editReply({ content: ":no_entry_sign: Tu ne sembles pas avoir la permission de modifier ce sondage", ephemeral: true })
                }
            ...
            if (interaction.customId.startsWith('sondageCmd-editQuestion-')) {
                ...
                await interaction.editReply({ content: "Écrivez la nouvelle question dans un message. Dites \"cancel\" pour annuler.", ephemeral: true }).then(() => {
                    ...
                    collector.on('collect', async m => {
                        ...
                        // S'il annule
                        if (m.content.toLowerCase() == "cancel") return interaction.editReply({ content: "Annulé !", ephemeral: true })

                        // On modifie le sondage
                        sondage.title = m.content
                        await bacheroFunctions.database.set(database, 'sondage-' + sondageId, sondage)
            ...
            if (interaction.customId.startsWith('sondageCmd-editOption-')) {
                ...
                await interaction.editReply({ content: "Écrivez les choix dans un message, en les séparant par un \"|\". Dites \"cancel\" pour annuler.", ephemeral: true }).then(() => {
                    ...
                    collector.on('collect', async m => {
                        ...
                        var options = m.content.split('|').map(op => op.trim())
            ...
            if (interaction.customId.startsWith('sondageCmd-stop-')) {
                // S'il n'est pas autorisé à modifier le sondage on lui dit
                if (!interaction.channel.permissionsFor(interaction.member).has(PermissionFlagsBits.ManageMessages)) {
                    return interaction.editReply({ content: ":no_entry_sign: Tu ne sembles pas avoir la permission d'arrêter ce sondage", ephemeral: true })
                }
