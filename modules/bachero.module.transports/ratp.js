...
async function whenLineObtained(interaction, line, isBus){
	...
	if(cache_lines[line]) line = cache_lines[line] // si on l'a déjà en cache, on l'utilise
		...
		if(line.fetcherror) return await bacheroFunctions.report.createAndReply("requête vers l'API de la RATP (n°1)", line.fetcherror || line, { line }, interaction)
	...
	if(cache_stops[line.id]) var stops = cache_stops[line.id]
		...
		if(stops.fetcherror) return await bacheroFunctions.report.createAndReply("requête vers l'API de la RATP (n°2)", stops.fetcherror || stops, { lineId: line?.id }, interaction)
...
async function whenStopObtained(interaction, stop, isComplete){
	...
	if(times.fetcherror) return await bacheroFunctions.report.createAndReply("requête vers l'API de la RATP (n°2)", times.fetcherror || times, { stop }, interaction)
