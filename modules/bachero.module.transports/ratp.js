...
async function whenStopObtained(interaction, stop, isComplete){
	...
	if(!isComplete){
		...
		// Sinon on demande à l'utilisateur de préciser
			...
			const row = new ActionRowBuilder().addComponents( // On crée une action row
				new StringSelectMenuBuilder()
			...
			if(interaction.customId == 'ratpCmd-type'){
				...
				// Sinon
					...
					const row = new ActionRowBuilder().addComponents(
						...
						.addOptions(
							transportIds[type].map(t => new StringSelectMenuOptionBuilder().setLabel(t.name).setValue(t.id))
						)
					)
