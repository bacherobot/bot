...
module.exports = {
	...
	async execute(interaction){
		...
		.setColor(infos.online ? bacheroFunctions.config.getValue('bachero', 'embedColor') : bacheroFunctions.config.getValue('bachero', 'secondEmbedColor'))
		.setFooter({ text: `Sous la demande de ${interaction.user.discriminator == '0' ? escape(interaction.user.username) : escape(interaction.user.tag)}${cache_hit ? ' • Provient du cache' : ''}${cache_time_remaining ? `, S'actualise dans ${cache_time_remaining} seconde${cache_time_remaining > 1 ? 's' : ''}` : ''}` })
